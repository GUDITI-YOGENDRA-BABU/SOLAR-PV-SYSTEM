...
%% === Load Dataset (GRID CONNECTED) ===
genData = readtable('C:\Users\gudit\PVsyst7.0_Data\UserHourly\GRID CONNECTED WITHOUT SHADING_Project_VC0_HourlyRes_1.CSV');
disp(genData)
genData.date = datetime(genData.date,'InputFormat','dd-MM-yyyy HH:mm:ss');

% Valid operating condition
genData = genData(genData.GlobInc > 0 & genData.E_Grid > 0, :);
disp(genData)
...
y = genData.E_Grid;   % Grid injected AC power

%% === STANDARDIZATION (VERY IMPORTANT) ===
X = normalize(X);   % <-- FIX FOR WARNING
...
figure;
plot(yTest(1:100),'k','LineWidth',1.5); hold on;

colors = lines(numModels);
for i = 1:numModels
    plot(yPreds(1:100,i),'--','Color',colors(i,:),'LineWidth',1.2);
end

legend(['Actual', models(:,1)'], 'Location','northwest');
xlabel('Sample Index');
ylabel('AC Power (W)');
title('Grid-Connected Without shading PV: Actual vs Predicted AC Power');
grid on;
